...
function findGender(array , gender){
    return fileterGame = array.filter((game) => game.genders.includes(gender))
      
}

const adventureGames = findGender(videogames, 'Aventura')

console.log(adventureGames)

const Scores = adventureGames.map((game) => game.score)
console.log(Scores)

const mediaScores = Scores.reduce((acc, next,index,array) => { 
     acc += next;
