// Usa un bucle y dos condiciones para imprimir por consola el nombre de
// los usuarios que sean menor de edad precedidos del texto
// "Usuarios menores de edad:" y otro que imprima a los usuarios mayores de edad,
// precedido del texto "Usuarios mayores de edad:".

const users = [
    { name: "Tony", years: 43 },
    { name: "Peter", years: 18 },
    { name: "Natasha", years: 14 },
    { name: "Bruce", years: 32 },
    { name: "Khamala", years: 16 },
    ];

function Edades(u){
    const mayores = []
    const menores = []
    for(p of u){
        if(p.years >= 18){mayores.push(p.name)}
        else{
            menores.push(p.name)
    }}
    
    return `Los usuarios mayores de edad son: ${mayores}`+` Los usuarios menores son: ${menores}`
    // return  u.map((user)=>{if(user.years >= 18){return user.name}})
}   


console.log(Edades(users))

